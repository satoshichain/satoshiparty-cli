...
from shellpartycli.wallet import SatoshiChaincore, shellwallet
from shellpartylib.lib import config, util, exceptions, script
from shellpartycli.util import api, value_out
...
def WALLET():
    return sys.modules['shellpartycli.wallet.{}'.format(config.WALLET_NAME)] 

def get_wallet_addresses():
    return WALLET().get_wallet_addresses()

def get_shell_balances():
    for address, shell_balance in WALLET().get_shell_balances():
    	yield [address, shell_balance]
...
def get_shell_balance(address):
    return WALLET().get_shell_balance(address)
