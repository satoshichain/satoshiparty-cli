...
from shellpartylib.lib import script, config, blocks, exceptions, api, transaction
from shellpartylib.lib.util import make_id, BET_TYPE_NAME, BET_TYPE_ID, dhash, generate_asset_name
from shellpartylib.lib.kickstart.utils import ib2h
from shellpartycli import util
from shellpartycli import wallet

import SatoshiChain as SatoshiChainlib
...
MESSAGE_PARAMS = {
    ...
    'shellpay': ['source', 'order_match_id'],
...
def get_input_value(tx_hex):
    ...
    ctx = SatoshiChainlib.core.CTransaction.deserialize(binascii.unhexlify(tx_hex))
