...
from shellpartylib.lib.util import value_input, value_output
...
def api(method, params=None):
    return rpc(config.SCHPARTY_RPC, method, params=params, ssl_verify=config.SCHPARTY_RPC_SSL_VERIFY)
...
def is_divisible(asset):
    if asset in (config.SCH, config.SHP, 'leverage', 'value', 'fraction', 'price', 'odds'):
        return True
    else:
...
def bootstrap(testnet=False, overwrite=True, ask_confirmation=False):
    ...
    # Set Constants.
    if testnet:
        BOOTSTRAP_URL = 'https://s3.amazonaws.com/shellparty-bootstrap/shellparty-db-testnet.latest.tar.gz'
        TARBALL_PATH = os.path.join(tempfile.gettempdir(), 'shellpartyd-testnet-db.latest.tar.gz')
        DATABASE_PATH = os.path.join(data_dir, '{}.testnet.db'.format(config.APP_NAME))
    else:
        BOOTSTRAP_URL = 'https://s3.amazonaws.com/shellparty-bootstrap/shellparty-db.latest.tar.gz'
        TARBALL_PATH = os.path.join(tempfile.gettempdir(), 'shellpartyd-db.latest.tar.gz')
    ...
    with tarfile.open(TARBALL_PATH, 'r:gz') as tar_file:
        ...
        safe_extract(tar_file, path=data_dir)
...
def add_config_arguments(arg_parser, config_args, default_config_file, config_file_arg_name='config_file'):
    ...
    if not config_file:
        config_dir = appdirs.user_config_dir(appauthor=config.SHP_NAME, appname=config.APP_NAME, roaming=True)
