...
from shellpartylib.lib.util import make_id, BET_TYPE_NAME
from shellpartylib.lib.log import isodt
from shellpartylib.lib.exceptions import TransactionError
from shellpartycli.util import add_config_arguments
from shellpartycli.setup import generate_config_files
from shellpartycli import APP_VERSION, util, messages, wallet, console, clientapi

APP_NAME = 'shellparty-client'

CONFIG_ARGS = [
    [('-v', '--verbose'), {'dest': 'verbose', 'action': 'store_true', 'help': 'sets log level to DEBUG instead of WARNING'}],
    [('--testnet',), {'action': 'store_true', 'default': False, 'help': 'use {} testnet addresses and block numbers'.format(config.SCH_NAME)}],    
    [('--testcoin',), {'action': 'store_true', 'default': False, 'help': 'use the test {} network on every blockchain'.format(config.SHP_NAME)}],
    
    [('--shellparty-rpc-connect',), {'default': 'localhost', 'help': 'the hostname or IP of the Shellparty JSON-RPC server'}],
    [('--shellparty-rpc-port',), {'type': int, 'help': 'the port of the Shellparty JSON-RPC server'}],
    [('--shellparty-rpc-user',), {'default': 'rpc', 'help': 'the username for the Shellparty JSON-RPC server'}],
    [('--shellparty-rpc-password',), {'help': 'the password for the Shellparty JSON-RPC server'}],
    [('--shellparty-rpc-ssl',), {'default': False, 'action': 'store_true', 'help': 'use SSL to connect to the Shellparty server (default: false)'}],
    [('--shellparty-rpc-ssl-verify',), {'default': False, 'action': 'store_true', 'help': 'verify SSL certificate of the Shellparty server; disallow use of self‐signed certificates (default: false)'}],

    [('--wallet-name',), {'default': 'SatoshiChaincore', 'help': 'the wallet name to connect to'}],
    ...
    [('--wallet-user',), {'default': 'SatoshiChainrpc', 'help': 'the username used to communicate with wallet'}],
    ...
    [('--fee-per-kb',), {'type': D, 'default': D(config.DEFAULT_FEE_PER_KB / config.UNIT), 'help': 'fee per kilobyte, in {}'.format(config.SCH)}],
    [('--regular-dust-size',), {'type': D, 'default': D(config.DEFAULT_REGULAR_DUST_SIZE / config.UNIT), 'help': 'value for dust Pay‐to‐Pubkey‐Hash outputs, in {}'.format(config.SCH)}],
    [('--multisig-dust-size',), {'type': D, 'default': D(config.DEFAULT_MULTISIG_DUST_SIZE / config.UNIT), 'help': 'for dust OP_CHECKMULTISIG outputs, in {}'.format(config.SCH)}],
    [('--op-return-value',), {'type': D, 'default': D(config.DEFAULT_OP_RETURN_VALUE / config.UNIT), 'help': 'value for OP_RETURN outputs, in {}'.format(config.SCH)}],
