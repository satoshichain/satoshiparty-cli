...
def extract_old_config():
    ...
    old_appdir = appdirs.user_config_dir(appauthor='Shellparty', appname='shellpartyd', roaming=True)
    old_configfile = os.path.join(old_appdir, 'shellpartyd.conf')
...
def extract_SatoshiChaincore_config():
    SatoshiChaincore_config = {}

    # Figure out the path to the SatoshiChain.conf file
    if platform.system() == 'Darwin':
        shell_conf_file = os.path.expanduser('~/Library/Application Support/Shellcoin/')
    elif platform.system() == 'Windows':
        shell_conf_file = os.path.join(os.environ['APPDATA'], 'Shellcoin')
    else:
        shell_conf_file = os.path.expanduser('~/.SatoshiChain')
    shell_conf_file = os.path.join(shell_conf_file, 'SatoshiChain.conf')

    # Extract contents of SatoshiChain.conf to build service_url
    if os.path.exists(shell_conf_file):
        conf = {}
        with open(shell_conf_file, 'r') as fd:
            # Shellcoin Core accepts empty rpcuser, not specified in shell_conf_file
            ...
            for SatoshiChaind_key in config_keys:
                if SatoshiChaind_key in conf:
                    shellparty_key = config_keys[SatoshiChaind_key]
                    SatoshiChaincore_config[shellparty_key] = conf[SatoshiChaind_key]

    return SatoshiChaincore_config

def get_server_known_config():
    server_known_config = {}

    SatoshiChaincore_config = extract_SatoshiChaincore_config()
    server_known_config.update(SatoshiChaincore_config)
...
def server_to_client_config(server_config):
    ...
    config_keys = {
        ...
        'rpc-host': 'shellparty-rpc-connect',
        'rpc-port': 'shellparty-rpc-port',
        'rpc-user': 'shellparty-rpc-user',
        'rpc-password': 'shellparty-rpc-password'
    }
...
def generate_config_files():
    from shellpartycli.server import CONFIG_ARGS as SERVER_CONFIG_ARGS
    from shellpartycli.client import CONFIG_ARGS as CLIENT_CONFIG_ARGS
    from shellpartylib.lib import config, util

    configdir = appdirs.user_config_dir(appauthor=config.SHP_NAME, appname=config.APP_NAME, roaming=True)
...
def before_py2exe_build(win_dist_dir):
    ...
    for exe_name in ['client', 'server']:
        shutil.copy('shellpartycli/__init__.py', 'shellparty-{}.py'.format(exe_name))
        with open('shellparty-{}.py'.format(exe_name), 'a') as fp:
...
def after_py2exe_build(win_dist_dir):
    ...
    for exe_name in ['client', 'server']:
        os.remove('shellparty-{}.py'.format(exe_name))
    # py2exe copies only pyc files in site-packages.zip
    # modules with no pyc files must be copied in 'dist/library/'
    import shellpartylib, certifi
    additionals_modules = [shellpartylib, certifi]
...
def bootstrap(overwrite=True, ask_confirmation=False):
    if ask_confirmation:
        question = 'Would you like to bootstrap your local Shellparty database from `https://s3.amazonaws.com/shellparty-bootstrap/`? (y/N): '
